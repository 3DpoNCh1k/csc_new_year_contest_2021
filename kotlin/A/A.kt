...
fun my_convolve(f: Function, g: Function,
                fFrom: Double, fTo: Double,
                gFrom: Double, gTo: Double,
                numPointsToCalc: Int = 100, numPointsInIntegral: Int = 100,
                ar: ArrayList<ArrayList<Pair<Double,Double>>> = ArrayList(),
                ary: ArrayList<Double> = ArrayList()): Function {
    val fxs = pointsFromRange(fFrom, fTo, numPointsToCalc)
    val sz = (((gTo-gFrom) / (fTo-fFrom)) * numPointsToCalc).toInt()
    println("sz = $sz")
    val gxs = pointsFromRange(gFrom, gTo, sz)
    require(fxs.size > 1)
    ...
    val ys = List(n) {
        ...
        for (i in 0 until numPointsToCalc) {
            val j = it - i
            if (j >= 0 && j < sz) {
                res += f(fxs.get(i)) * g(gxs.get(j)) * step
            }
        }
        res
    }
...
fun Function.integrate(From: Double, To: Double, numPointsInIntegral: Int = 10000): Double {
    val INF = 1e9
    val g = interpolate(listOf(-INF, INF), listOf(1.0, 1.0))
    return my_convolve(this, g, From, To, 0.0, To-From, numPointsInIntegral, numPointsInIntegral=numPointsInIntegral)(To)
}

fun solve(n: Int, xs: List<Double>, ys: List<Double>): List<Double> {
    var d=1.0
    var f = interpolate(xs,ys)
    var f2 = f.interpolate(0.0,1.0)
    return xs
}
...
//    val y: List<Double> = readLine()!!.split(" ").map { it.toDouble() }
    ...
    val res: List<Double> = solve(n, x, y)
